        ...
        PATIENT INFORMATIONS</h6>
            ...
            if(isset($_GET['id']))
            {
                ...
                $query = "SELECT u.*, p.due_date, p.pregnancy_status 
                          FROM users u 
                          LEFT JOIN pregnancy_informations p ON u.id = p.patient_id 
                          WHERE u.id = '$user_id' AND u.doctor_id = '{$_SESSION['id']}' 
                          GROUP BY u.id";
  ...
  <p class="text-gray-600">Personal Informations</p> 
                    <div class="form-group row small">
                        <div class="col-sm-4 mb-sm-0">
                            ...
                            <input type="text" value = "<?=$row['firstname'];?>" required class="form-control" placeholder="First Name" readonly>
                        </div>
                        <div class="col-sm-4">
                            <label>Middle Initial</label>
                            <input type="text" value = "<?=$row['middlename'];?>" required class="form-control" placeholder="Last Name" readonly>
                        </div>
                        <div class="col-sm-4">
                            <label>Last Name</label>
                            <input type="text" value = "<?=$row['lastname'];?>" required class="form-control" placeholder="Middle Initial" readonly>
                        </div>
                    </div>
                    ...
                    <div class="form-group row small">
                        <div class="col-sm-4">
                        <label>Age</label>
                        <input type="number" value = "<?=$row['age'];?>" class="form-control" placeholder="Age" readonly>
                        </div>
                        <div class="col-sm-8">
                        <label>Address</label>
