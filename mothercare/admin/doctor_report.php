...
?>
 ...
 <div class="d-sm-flex align-items-center justify-content-between mb-4 ml-1">
    ...
    <a href="doctor_report.php" class="btn btn-sm btn-info shadow-sm" value="print" onclick="PrintDiv();">
                                ...
                                <div class="col-md-4">
                                    <div class="form-group">
                                        ...
                                        <select name="name_id" class="form-control">
                                            ...
                                            <?php
                                            $query = "SELECT speciality_name, id FROM specialities";
                                            ...
                                            while ($row = mysqli_fetch_assoc($result)) {
                                                ...
                                                echo "<option value='{$row['id']}' {$selected}>{$row['speciality_name']}</option>";
                                            }
                                            ?>
                                        </select>
                                    </div>
                ...
                <div class="card-body py-0">
                    <div class="table-responsive">
                        <table class="table table-sm table-bordered table-hover" width="100%" cellspacing="0">
                           ...
                           <!--  <tfoot class='thead-light' style="text-align:center">
                                    <th>No.</th>
                                    <th>Patient Name</th>
                                    <th>Age</th>
                                    <th>Gender</th>
                            ...
                            <?php 
                                if(isset($_GET['from_date']) && isset($_GET['to_date']))
                                {
                                    ...
                                    $no=1;
                                    $query = "SELECT doctors.*, specialities.speciality_name 
                                              FROM doctors 
                                              LEFT JOIN specialities ON doctors.speciality_id = specialities.id";

                                    if (isset($_GET['name_id']) && $_GET['name_id'] !== '') {
                                        $namefilter = "AND specialities.id = '{$_GET['name_id']}'";
                                        $query .= " WHERE 1=1 $namefilter";
                                    }
                                    
                                    if (isset($_GET['from_date']) && isset($_GET['to_date'])) {
                                        $query .= " AND doctors.date_created BETWEEN '$from_date' AND '$to_date'";
                                    }
